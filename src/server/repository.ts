...
export interface IRepository {
    ...
    replace(uid: string, bid: string, meta: Metadata, blob: Buffer): Promise<void>;
    remove(uid: string, bid: string, meta: Metadata): Promise<void>;
}

export class AzureStorageRepository implements IRepository{
    private serviceClient = BlobServiceClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING);
    private containerClient = this.serviceClient.getContainerClient(process.env.AZURE_STORAGE_CONTAINER_NAME);
    private tableClient = TableClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING, process.env.AZURE_STORAGE_CONTAINER_NAME);
    ...
    async replace(uid: string, bid: string, meta: Metadata, blob?: Buffer): Promise<void> {
        if (blob) {
...
export type LogReason = 'add' | 'replace' | 'remove' | 'extend' | 'authorized' | 'unauthorized';  

export type LogRecord = Omit<TableEntityResult<{
    uid: string
    bid: string
    status: string
    reason: string
...
export interface ILogger {
    ...
    log(uid: string, bid: string, title: string, status: 'close'|'open', reason: LogReason, ip: string): Promise<void>;
}

export class AzureStorageLogger implements ILogger {
    private tableClient = TableClient.fromConnectionString(process.env.AZURE_STORAGE_CONNECTION_STRING, process.env.AZURE_STORAGE_LOG_TABLE_NAME);
    
    async list(uid: string, skiptoken: string, pageSize: number): Promise<LogRecord[]> {
        const result: LogRecord[] = []
        const pages = this.tableClient.listEntities<LogRecord>({
            queryOptions: { filter: odata`PartitionKey eq ${uid} and RowKey gt ${skiptoken}`}
    ...
    async log(uid: string, bid: string, title: string, status: "close" | "open", reason: LogReason, ip: string): Promise<void> {
    ...
    async delall(uid: string, skiptoken: string, pageSize: number): Promise<LogRecord[]> {
        const result: LogRecord[] = []
        const pages = this.tableClient.listEntities<LogRecord>({
            queryOptions: { filter: odata`PartitionKey eq ${uid} and RowKey gt ${skiptoken}`}
        });
