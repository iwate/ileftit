...
const fetcher = url => fetch(url).then(res => res.ok ? res.json().then(o => ({...o, state: 'open'})) : { state: 'close' });

export default function Page({state}) {
  const { t, locale } = useLocale();
  const fmt = new Intl.DateTimeFormat(locale, { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' });
    ...
    <main>
      ...
      <section className="form-vertical">
        <dl>
          <dt></dt>
          <dd>
            <label className={themeFont.className}>for
        ...
        <dl>
          <dt></dt>
          <dd>
            <textarea rows={15} readOnly defaultValue={data.body}></textarea>
          </dd>
        </dl>
      </section>:
      data.status === "open"?
      <form className="form-vertical" onSubmit={handleSubmit(onSubmit)}>
        <dl>
          ...
          <dd>
            ...
            <input type="password" placeholder={t.PlaceholderViewPassword} {...register("secret", {required: true})}/>
            {errors.secret?.type === 'required' && <p>{t.ValidationRequired}</p>}
          </dd>
        </dl>
        <dl>
          <dt></dt>
          <dd>
            <Button type="submit" loading={loading==='show'} disabled={loading}>{t.ActionShow}</Button>
          </dd>
        </dl>
      </form>:
      data.status === "close" && data.verified ?
      <section>
        <p className="text-center">{t.MessageCloseVerified.replaceAll('{0}', fmt.format(new Date(data.openAt)))}</p>
      </section>:
      <section>
