...
const getKey = (pageIndex, prev) => {
  ...
  return `/api/logs?size=${PAGE_SIZE}&skiptoken=${prev.items[prev.items.length-1].rowKey}`;
}
function History() {
  const { t, locale } = useLocale();
  const fmth = new Intl.DateTimeFormat(locale, { year: 'numeric', month: '2-digit', day: '2-digit' });
  const fmtl = new Intl.DateTimeFormat(locale, { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  ...
  return <table className="history">
    ...
    <tbody>
      {data.flatMap(o => o.items).map(o => <tr key={o.rowKey}>
        <td>{fmth.format(new Date(o.timestamp))}<br/>{fmtl.format(new Date(o.timestamp))}</td>
        <td>{o.title}<br/>({o.bid.substring(0,3) + '...' + o.bid.substring(o.bid.length-3)})</td>
        <td>{o.status}</td>
        <td>{o.reason}</td>
        <td>{o.ip}</td>
      </tr>)}
    </tbody>
    {data.length > 0 && data[data.length - 1].items.length === PAGE_SIZE && <tfoot>
      <tr>
        <td colSpan={5}>
          <Button type="button" looks="text" onClick={more} loading={isLoading}>{t.ActionMore}</Button>
        </td>
      </tr>
    </tfoot>}
  </table>
...
const saveExtendHours = (value: number) => localStorage.setItem('extend_hours', String(value));
...
function OpenAt({date}: {date:Date}) {
  const { t, locale } = useLocale();
  const fmt1 = new Intl.DateTimeFormat(locale, { year: 'numeric', month: '2-digit', day: '2-digit' });
  const fmt2 = new Intl.DateTimeFormat(locale, { hour: '2-digit', minute: '2-digit', second: '2-digit' });
  const normalize = (str: string) => str.replace(/\s/g, ' ');
  const now = new Date();
  const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
  const dt = date.getTime() - today.getTime();
  return <small>
    {t.LabelOpenAt} {normalize(dt > 0 && dt < $1day ? fmt2.format(date) : fmt1.format(date))}
  </small>
}
...
export default function Home({items}) {
  ...
  }, [setExtendHours, loadExtendHours]);
  ...
  const onSubmit = (bids:string[]): FormEventHandler<HTMLFormElement> => {
    return e => {
      ...
      extend(bids, extendHours).then(data => {
        router.push('/wills/');
      }).catch(() => {
    ...
    <main className="home">
      <section className="will-list">
            ...
            <summary>
              ...
              <form className="form-horizontal" onSubmit={onSubmit([will.bid])}>
                ...
                <dl>
                  <dt></dt>
                  <dd>
                      <select name="extend" value={extendHours} onChange={updateExtendHours}>
                ...
                <dl>
                  ...
                  <Button type="submit"
                              ...
                              disabled={loading}>{t.ActionExtend}</Button>
                    <Link href={`/wills/${will.bid}/@replace`} className="btn-text">{t.ActionReplace}</Link>
                    <Link href={`/wills/${will.bid}/@delete`} className="btn-text">{t.ActionDelete}</Link>
                  </dd>
                </dl>
              </form>
            </details>
        ...
        {items.length < 10 && (
          <div className="will-list-footer">
            <Link href={`/wills/@create`} className="btn-text">+ {t.ActionAdd}</Link>
          </div>
        )}
      </section>
      <section className="pr">
          <ul>
            <li>
              ...
              <Link href="https://github.com/sponsors/iwate">https://github.com/sponsors/iwate</Link>
            </li>
          </ul>
      </section>
      {items.length > 0&&
        <form className="form-horizontal right" onSubmit={onSubmit(items.map(will => will.bid))}>
          <dl className="w100">
            <dt></dt>
            <dd>
              <div className="btn-group w100">
                ...
                <Button type="submit"
                      ...
                      disabled={loading}>{t.ActionExtendAll}</Button>
              </div>
            </dd>
          </dl>
        </form>}
      ...
      <section className="sponsors">
        ...
        <ul>
          ...
          <li>@username</li>
          <li>...</li>
          <li>All sponsors can be listed here. <Link href="https://github.com/sponsors/iwate">https://github.com/sponsors/iwate</Link></li>
        </ul>
      </section>
    </main>
  )
