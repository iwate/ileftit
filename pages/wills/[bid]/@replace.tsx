      ...
      <form className="form-vertical" onSubmit={handleSubmit(onSubmit)}>
        <dl>
          ...
          <dd>
            ...
            <input placeholder={t.PlaceholderFor} {...register("title", {required: true, maxLength: 20})}/>
            </label>{
            errors.title?.type === 'required' && <p>{t.ValidationRequired}</p>}
            {errors.title?.type === 'maxLength' && <p>{t.ValidationMaxLength.replaceAll('{0}', '20')}</p>}
          </dd>
        </dl>
        <dl>
          <dt></dt>
          <dd>
            <textarea placeholder={t.PlaceholderReplaceText} rows={15} {...register("body", {required: true, maxLength: 10000})}></textarea>
            {errors.body?.type === 'required' && <p>{t.ValidationRequired}</p>}
            {errors.body?.type === 'maxLength' && <p>{t.ValidationMaxLength.replaceAll('{0}', '10000')}</p>}
          </dd>
        </dl>
        <dl>
          <dt></dt>
          <dd>
            <input type="password" placeholder={t.PlaceholderReplacePassword} {...register("secret", {required: true})}/>
        ...
        <dl>
          <dt></dt>
          <dd>
            <Button type="submit" loading={loading==='replace'} disabled={loading}>{t.ActionReplace}</Button>
          </dd>
        </dl>
        <dl>
          <dt></dt>
